...
pkgs.writeShellScriptBin "clear-testvm" ''
  sed --in-place --follow-symlinks "/10.10.1.19/d" ${config.programs.ssh.settings."*".data.userKnownHostsFile};
  sudo sed --in-place --follow-symlinks "/10.10.1.19/d" /root/.ssh/known_hosts
  sed --in-place --follow-symlinks "/testvm/d" ${config.programs.ssh.settings."*".data.userKnownHostsFile};
