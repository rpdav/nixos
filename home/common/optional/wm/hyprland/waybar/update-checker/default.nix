...
pkgs.writeShellScript "update_checker" ''

  # ===== Configuration =====
  UPDATE_INTERVAL=3599  # Check interval in seconds (1 hour)
  NIXOS_CONFIG_PATH="${config.home.homeDirectory}/nixos"  # Path to NixOS configuration
  CACHE_DIR="${config.home.homeDirectory}/.cache"
  ...
  function send_notification() {
      ...
      ${pkgs.libnotify}/bin/notify-send -i "$HOME/.icons/$icon.png" "$title" "$message" $expire_flag
  }
  ...
  function check_for_updates() {
      ...
      if [ "$UPDATE_LOCK_FILE" = "true" ]; then
          ...
          if [ "$?" -eq 0 ]; then
              updates=$(${pkgs.nvd}/bin/nvd diff /run/current-system ./result | grep -e '\[U' | wc -l)
              tooltip=$(${pkgs.nvd}/bin/nvd diff /run/current-system ./result | grep -e '\[U' | awk '{ for (i=3; i<NF; i++) printf $i " "; if (NF >= 3) print $NF; }' ORS='\\n')
          else
              return 1
          fi
      else
