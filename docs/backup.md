...
## User backup
User backup is in `homeModules.backup`, which backs up user directories using `borgmatic`. This differs from the system modules which use `borg`. For now, user backup is local-only.
