# Virtualization
My `nas` system is also a virtualization host running a windows VM with hardware passthrough (an NVME drive, GPU, and audio and USB controllers). I use [nixvirt](https://github.com/AshleyYakeley/NixVirt) which allows libvirt networks and domains (that is, VMs) to be configured with nix. 
...
* If you're doing GPU passthrough (and so won't have access to the host's display), you can run `virt-manager` on another machine and connect to the VM host through ssh.
* If installing windows, leave the network interface inactive in during install (or use `virtio` networking which won't work without drivers) in order to bypass the account creation requirement.
...
* [This tool](https://passthroughtools.org/cpupin/) is helpful in determining the best way to pass through CPU cores.
